...
describe('CratesService', () => {
  ...
  const sampleSubmission: CrateSubmission = {
    id: 'submission1',
    crateID: 'crate1',
    trackId: 'track1',
    fromID: 'user2',
    message: 'Please add my track to your crate!',
    status: 'pending',
    createdAt: new Date(),
    updatedAt: new Date(),
  };
  ...
  describe('updateCrate', () => {
    ...
    it('returns error if updateData invalid', async () => {
      ...
      vi.mocked(mockRepo.update).mockResolvedValueOnce(
        err('No valid fields to update')
      );
      const updateData: any = {
        name: 124325, // Invalid type
      };
      const result = await cratesService.updateCrate(
        'crate1',
        updateData,
  ...
  describe('deleteCrate', () => {
    it('deletes crate successfully', async () => {
      ...
      const result = await cratesService.deleteCrate('crate1', 'user1');
      expect(result.ok).toBe(true);
      if (result.ok) {
    ...
    it('prevents deletion if user doesnt own the crate', async () => {
      vi.mocked(mockRepo.getById).mockResolvedValueOnce(ok(sampleCrate));
      const result = await cratesService.deleteCrate('crate1', 'user2');
      expect(result.ok).toBe(false);
      if (!result.ok) {
  ...
  describe('addTrackToCrate', () => {
    ...
    it('returns error if track already exists', async () => {
      vi.mocked(mockRepo.getById).mockResolvedValueOnce(ok(sampleCrate));
      vi.mocked(mockRepo.addTrack).mockResolvedValueOnce(
        ok('Track already added')
      );
      const result = await cratesService.addTrackToCrate('crate1', 'track1');
      expect(result.ok).toBe(true);
      if (!result.ok) {
        expect(result.error).toBe('Track already added');
      }
    });
    ...
    it('returns error on repo failure', async () => {
      vi.mocked(mockRepo.getById).mockResolvedValueOnce(ok(sampleCrate));
      vi.mocked(mockRepo.addTrack).mockResolvedValueOnce(
        err('Failed to add track')
      );
      const result = await cratesService.addTrackToCrate('crate1', 'track1');
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error).toBe('Failed to add track');
      }
      expect(mockRepo.addTrack).toHaveBeenCalledWith('crate1', 'track1');
    });
  });
  ...
  describe('removeTrackFromCrate', () => {
    ...
    it('returns error on repo failure', async () => {
      ...
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error).toBe('Failed to remove track');
      }
    });
  });
  ...
  describe('getTracksInCrate', () => {
    ...
    it('returns error if crateId invalid', async () => {
      ...
      expect(result).toEqual(err('Invalid crate ID'));
  ...
  describe('acceptTrackSubmission', () => {
    it('accepts submission successfully', async () => {
      ...
      expect(mockRepo.resolveSubmission).toHaveBeenCalledWith(
        'submission1',
        'accepted'
      );
    });
